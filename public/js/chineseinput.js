 ...
 */
/* fix jslint errors
 * add input text support
 * */
(function ($) {
    'use strict';

    var caretClass = 'textarea-helper-caret', dataKey = 'textarea-helper',
    // Styles that could influence size of the mirrored element.
        mirrorStyles = [
    // Box Styles.
            'box-sizing', 'height', 'width', 'padding-bottom', 'padding-left',
    ...
    prototypehelper = function () {
        this.update = function () {

            // Copy styles.
            var styles = {}, i, style, caretPos, str, pre, post, $car;
            for (i = 0; i < mirrorStyles.length; i++) {
                style = mirrorStyles[i];
        ...
        this.caretPos = function () {
            this.update();
            var $caret = this.$mirror.find('.' + caretClass), pos = $caret.position();
    ...
    prototypehelper.call(TextareaHelper.prototype);

    $.fn.textareaHelper = function (method) {
        this.each(function () {
            var $this = $(this), instance = $this.data(dataKey);
            if (!instance) {
...
(function ($) {
    ...
    $.chinesePinyinInput = function (el, conf) {
        ...
        self.addTxtAtCusor = function (text) {
            ...
            if (document.selection) {
                ...
                start = self.el.selectionStart;
                end = self.el.selectionEnd;
        ...
        self.chooseCandidate = function (selectionIndex) {
            var choices = self.hanzis, choice, len;
        ...
        self.refresh = function () {
            ...
            var candidates = self.getCandidates(), $box, lis, i, caretPosition;
            if (candidates && candidates.length) {
                $box = self.getPinyinBar();
                $box.find('.pinyin').text(self.pinyinInput);
                lis = [];
                for (i = 0; i < 5 && i < candidates.length; i++) {
                    lis
                            .push('<li '
                ...
                $box.show();
                caretPosition = self.$el.textareaHelper('caretPos');
                $box.css({
                    position : 'absolute',
        ...
        self.getCandidates = function () {
            ...
            if (candidates && candidates[candidates.length - 1] === self.pinyinInput) {
                self.lastPage = true;
                return candidates.slice(self.currentPage * 5);
            }
            return false;
        ...
        self.requestPinyinCandiates = function (text, page) {
            var num = 10 + parseInt(Math.floor(page / 2), 10) * 10;
            $
                    .get(
        ...
        self.parseData = function (data) {
            ...
            hanzis = reply[1];
            self.lens = reply[2];
            if (self.lens === undefined) {
                self.lens = [];
                self.lens.length = hanzis.length;
                for (i = 0; i < self.lens.length; i++) {
    ...
    $.fn.chinesePinyinInput = function (conf) {
        return this.each(function () {
            var pinyin = (new $.chinesePinyinInput(this, conf));
            $(this).data("ChinesePinyinInput", pinyin);
        });
    };
}(jQuery));
